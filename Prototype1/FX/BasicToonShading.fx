...
//identical to Basic.fx, but adapted to allow for toon shading
...
float4 PS(VertexOut pin, uniform int gLightCount, uniform bool gUseTexure, uniform bool gAlphaClip, uniform bool gFogEnabled) : SV_Target
{
    ...
    if (gLightCount > 0)
    {
        ...
        float4 resultingMultiplier = (ambient + diffuse);
        float magnitude = sqrt(pow(resultingMultiplier.r, 2) + pow(resultingMultiplier.g, 2) + pow(resultingMultiplier.b, 2));
        
        if (magnitude < 0.3 && magnitude != 0)
        {
            resultingMultiplier /= float4(magnitude, magnitude, magnitude, 1);
            resultingMultiplier = float4(0.4, 0.4, 0.4, 1);
        }
        else if (magnitude > 0.7)
        {
            resultingMultiplier /= float4(magnitude, magnitude, magnitude, 1);
            resultingMultiplier = float4(1, 1, 1, 1);
        }
        else if (magnitude != 0)
        {
            float normalised = (magnitude - 0.3) / (0.7 - 0.3);
            float newMagnitude = 0.6 * normalised + 0.4;
            resultingMultiplier /= float4(magnitude, magnitude, magnitude, 1);
            resultingMultiplier = float4(newMagnitude, newMagnitude, newMagnitude, 1);
        }
