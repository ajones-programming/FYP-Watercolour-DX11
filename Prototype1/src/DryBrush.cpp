...
ID3D11ShaderResourceView* DryBrushFilter::GetOutput()
{
...
void DryBrushFilter::BlurInPlace(ID3D11DeviceContext* dc,
	...
	int blurCount)
{
	...
	for (int i = 0; i < blurCount; ++i)
	{
		...
		Effects::DryBrushFX->DryBrushTech->GetDesc(&techDesc);
		for (UINT p = 0; p < techDesc.Passes; ++p)
		{
			Effects::DryBrushFX->SetInputMap(inputSRV);
			Effects::DryBrushFX->SetOutputMap(mBlurredOutputTexUAV);
			Effects::DryBrushFX->SetOverlayMap(mOverlayTexSRV);
			Effects::DryBrushFX->DryBrushTech->GetPassByIndex(p)->Apply(0, dc);
		...
		// VERTICAL blur pass.
		Effects::DryBrushFX->ReturnTech->GetDesc(&techDesc);
		for (UINT p = 0; p < techDesc.Passes; ++p)
		{
			Effects::DryBrushFX->SetInputMap(mBlurredOutputTexSRV);
			Effects::DryBrushFX->SetOutputMap(inputUAV);
			Effects::DryBrushFX->SetOverlayMap(mOverlayTexSRV);
			Effects::DryBrushFX->DryBrushTech->GetPassByIndex(p)->Apply(0, dc);
