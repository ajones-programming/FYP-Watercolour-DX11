...
ToonShaderBasicEffect::ToonShaderBasicEffect(ID3D11Device* device, const std::wstring& filename)
	: Effect(device, filename)
{
	Light1Tech = mFX->GetTechniqueByName("Light1");
	Light2Tech = mFX->GetTechniqueByName("Light2");
...
ToonShaderBasicEffect::~ToonShaderBasicEffect()
{
}
#pragma endregion

#pragma region CSEffect
CSEffect::CSEffect(ID3D11Device* device, const std::wstring& filename, const char* horzEffect, const char* vertEffect)
	: Effect(device, filename)
{
	HorzTech = mFX->GetTechniqueByName(horzEffect);
	VertTech = mFX->GetTechniqueByName(vertEffect);
...
DryBrushEffect::DryBrushEffect(ID3D11Device* device, const std::wstring& filename)
	: Effect(device, filename)
{
	DryBrushTech = mFX->GetTechniqueByName("DryBrush");
	ReturnTech = mFX->GetTechniqueByName("Return");
...
ToonShaderBasicEffect* Effects::ToonShaderBasicFX = 0;
...
void Effects::InitAll(ID3D11Device* device)
{
	...
	ToonShaderBasicFX = new ToonShaderBasicEffect(device, L"FX/BasicToonShading.fxo");
	BlurFX  = new BlurEffect(device);
	MeanShiftFX = new MeanShiftEffect(device);
	DryBrushFX = new DryBrushEffect(device, L"FX/drybrush.fxo");
}
